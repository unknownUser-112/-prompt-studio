...
import { createSdxlLayout } from "../../../src/profiles/sdxl";
...
describe("V600 profile matrix", () => {
  ...
  it.each([
    GOLDEN_SCENARIOS.find(({ id }) => id === "baseline.de")!,
    GOLDEN_SCENARIOS.find(({ id }) => id === "baseline.en")!,
  ])("renders the SDXL baseline byte-identically: $id", async (scenario) => {
    const promptLanguage = scenario.language === "Deutsch" ? "Deutsch" : "English";
    const state = await new ConstraintEngine({ runtime: createFixedRuntime().runtime, stateBuilder: createResolvedStateBuilder() }).resolve(
      { ...createCanonicalProjectStateV5Values(), ...scenario.input, promptLanguage, profile: "SDXL", step: 9 },
      [
        additionalPersonProvider,
        adaptiveRealismProvider,
        brandProvider,
    ...
    const expected = matrix.entries.find((entry) => entry.scenarioId === scenario.id && entry.profileId === "sdxl")!.output;

    expect(new TextRenderer().render(document, createSdxlLayout(document, promptLanguage)).value).toBe(expected);
  });

  it.each(GOLDEN_SCENARIOS)("renders every SDXL scenario byte-identically: $id", async (scenario) => {
    const promptLanguage = scenario.language === "Deutsch" ? "Deutsch" : "English";
    const state = await new ConstraintEngine({ runtime: createFixedRuntime().runtime, stateBuilder: createResolvedStateBuilder() }).resolve(
      { ...createCanonicalProjectStateV5Values(), ...scenario.input, promptLanguage, profile: "SDXL", step: 9 },
      [
